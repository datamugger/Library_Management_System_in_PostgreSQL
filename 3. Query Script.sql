...
-- first changing the status to 'no' for issued book (as given data was not correct)
...
-- Since, we've to update a column, based on value of column of another table, we use JOIN during UPDATE
...
-- Now, some books are returned as well, so we've to set status='yes' for them in the book table
-- based on return_status table
...
-- Task is, to insert a record for book being returned in return_status table and 
...
FROM 
(
SELECT M.member_id, M.member_name, I.issued_book_isbn, I.issued_book_name, I.issued_date, R.return_date,
